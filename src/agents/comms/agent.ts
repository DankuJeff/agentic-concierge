 ...
 * Receives Decision Agent output (recommendation + action steps) and drafts
 * ready-to-use communications: cancellation emails/letters, enrollment emails,
 * and phone scripts with objection handling.
 */
...
import { buildCommsPrompt, PROMPT_VERSION } from './prompts/system.js';

const log = childLogger({ module: 'comms-agent' });

export class CommsAgent extends BaseAgent {
  readonly agentName = 'comms';

  async run(task: AgentTaskInput): Promise<AgentTaskOutput> {
    log.info(
      { taskId: task.taskId, action: task.action, promptVersion: PROMPT_VERSION },
      'Comms Agent starting',
    );

    const systemPrompt = buildCommsPrompt({
      communicationTone: task.userContext['communicationTone'] as string | undefined,
      name: task.userContext['name'] as string | undefined,
      currentDate: new Date().toISOString().slice(0, 10),
      gmailConnected: task.userContext['gmailConnected'] as boolean | undefined,
    });
    ...
    // Comms agent is single-turn — pure drafting, no tools.
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      ...
      if (!block || block.type !== 'text') {
        log.warn({ taskId: task.taskId, attempt }, 'Comms Agent returned no text');
      ...
      if (result.status === 'failed' && result.error?.includes('not valid JSON') && attempt < 3) {
        log.warn({ taskId: task.taskId, attempt }, 'Comms Agent output not valid JSON — retrying');
        continue;
      }

      log.info({ taskId: task.taskId, status: result.status }, 'Comms Agent completed');
      return result;
    }
  ...
  private parseAndValidate(taskId: string, rawText: string): AgentTaskOutput {
    ...
    try {
      ...
      log.error({ taskId, rawText: rawText.slice(0, 300) }, 'Comms Agent output is not valid JSON');
      return this.validateOutput({
        taskId,
        status: 'failed',
        sources: [],
